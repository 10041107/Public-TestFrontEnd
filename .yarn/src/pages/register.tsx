...
export default function Register() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.status === 201) {
        localStorage.setItem('token', response.data.accessToken); // 액세스 토큰 저장
        router.push('/login');
      } else {
  ...
  return (
    <div className="hide-scrollbar" style={{ height: '100vh', overflowY: 'scroll' }}>
      <div className="flex flex-col items-center w-screen h-screen py-6 my-10 mb-10 text-center bg-center bg-no-repeat mb-34 bg-summonersRift sm:py-8 lg:py-12">
        <AnimatePresence>
          {isOpen && (
            <>
              <motion.div
        ...
        {loading ? <p className="text-center">Loading...</p> : (
          <form className="grid max-w-screen-md gap-4 mx-auto sm:grid-cols-2" onSubmit={handleSubmit}>
            ...
            <label htmlFor="profileImage" className="inline-block mb-2 text-sm text-gray-800 sm:text-base text-neutral-700">프로필 이미지</label>
              ...
              <CldUploadWidget uploadPreset="p22qmytj" onUpload={handleImageUpload}>
                {({ open }) => (
