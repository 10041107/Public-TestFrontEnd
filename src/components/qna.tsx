...
import { QuestionOutlined, LoadingOutlined,CaretRightOutlined } from '@ant-design/icons';
...
const QuestionToggle: React.FC<QuestionToggleProps> = ({ question }) => {
  ...
  return (
    ...
    > 
      <Panel
        key="1"
        header={isExpanded ? <LoadingOutlined style={{ fontSize: '17px' }} /> : <QuestionOutlined style={{ fontSize: '17px' }} />}
        style={panelStyle}
      >
        <h4 className="text-sm text-neutral-500">{question.t}</h4>
      </Panel>
    </Collapse>
  );
};
...
const calculateEngagement = (shortAnswers: string[]): number => {
  ...
  const averageLength = totalLength / shortAnswers.length;

  const engagementScore = (averageLength / 300) * 100;
...
const calculateNeutrality = (answers: number[]): number => {
  const neutralAnswers = answers.filter(a => a === 2.5).length;
...
const renderQuestion = (question: QuestionType) => (
  <div className="hide-scrollbar" style={{ height: '100vh', overflowY: 'scroll' }}>
    <motion.div key={question.num} className="flex flex-col justify-center flex-grow w-full gap-10 mt-10 lg:gap-20 mb-14 lg:mb-0">
      <motion.div initial={{ opacity: 0, y: -50 }} animate={{ opacity: 1, y: 0, transition: { duration: 1 } }}>
        <h4 className="text-2xl lg:text-4xl text-neutral-600">{question.num}.</h4>
        <h2 className="my-4 text-neutral-700">{question.q}</h2>
        <QuestionToggle question={question} />
      </motion.div>
      <motion.ul initial={{ opacity: 0, y: -50 }} animate={{ opacity: 1, y: 0, transition: { duration: 1, delay: 0.5 } }} className="flex flex-col w-full gap-5 mb-20 lg:gap-8 lg:mb-32">
        {question.a.map((answer, idx) => (
          <li key={`${answer.option}${idx}`} onClick={() => handleAnswerClick(answer.score)} className="flex flex-col gap-2 px-5 py-4 duration-200 ease-in-out border-2 border-solid rounded-lg cursor-pointer border-neutral-500 lg:py-5 active:border-stone-300 active:border-2 active:bg-neutral-500 active:-translate-y-1">
            <h4 className="lg:text-xl text-neutral-600">{answer.text}</h4>
          </li>
        ))}
      </motion.ul>
    </motion.div>
  ...
  const renderShortAnswerQuestion = (question: string, index: number) => (
    ...
    <motion.div key={index} className="flex flex-col justify-center flex-grow w-full gap-10 mt-10 mb-10 lg:gap-10 lg:mb-0">
      <motion.div initial={{ opacity: 0, y: -50 }} animate={{ opacity: 1, y: 0, transition: { duration: 1 } }}>
        <h4 className="text-2xl lg:text-4xl text-neutral-600">{21 + index}.</h4>
        <h2>{question}</h2>
